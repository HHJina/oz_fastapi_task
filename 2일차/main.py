...
from app.schemas.movies import CreateMovieRequest, MovieUpdateRequest
...
def create_movie(request: CreateMovieRequest):
    movie = MovieModel.create(title=request.title, playtime=request.playtime, genre=request.genre)
    return movie
...
def update_movie(movie_id: int, request: MovieUpdateRequest):
    movie = MovieModel.get(id=movie_id)
    if movie is None:
        raise HTTPException(status_code=404)
...
def delete_movie(movie_id: int) -> dict[str, int]:
    movie = MovieModel.get(id=movie_id)
    if movie is None:
        raise HTTPException(status_code=404)
    movie.delete()

    return {"delete_id": movie_id}
